...
namespace EasyNow.Coin.App
{
    public class OkexService : IHostedService
    {
        ...
        public OkexService(IOptionsMonitor<OkexOptions> optionsMonitor, ILogger<OkexService> logger, ILifetimeScope lifetimeScope)
        {
        ...
        private void Transaction(string symbol)
        {
            ...
            var options = _optionsMonitor.CurrentValue;
            // 如果未设置参数则结束
            if (!options.Transaction.TryGetValue(symbol, out var parameters)||parameters.Length!=4)
            {
                return;
            }
